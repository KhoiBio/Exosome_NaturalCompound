...
RANKER_SYSTEM = """You are a cosmetic R&D decision analyst. Given retrieved PubMed literature,
rank the top natural compounds for the requested application.

For each compound return a JSON array. Each item must have these exact keys:
  compound, score (0-10 float), evidence_level, mechanism, safety_profile, novelty, summary

evidence_level: "High" / "Moderate" / "Low" / "Preliminary"
novelty: "High" / "Medium" / "Low"
safety_profile: one sentence
summary: one sentence product development insight

Respond ONLY with a valid JSON array. No markdown, no explanation outside the array."""

CLAIM_SYSTEM = """You are a cosmetic claims specialist and regulatory writer. Given scientific
literature about a compound and application, generate compliant cosmetic claim language.

Return a JSON object with these exact keys:
  primary_claim, supporting_claims (array of 3), evidence_basis, caution

primary_claim: the main marketing claim (≤12 words)
supporting_claims: 3 specific supporting statements with evidence basis
evidence_basis: "in vitro" / "in vivo" / "clinical" / "mixed"
caution: one sentence regulatory note

Respond ONLY with valid JSON. No markdown fences."""

READINESS_SYSTEM = """You are a cosmetic R&D lab scientist. Given PubMed literature about a
compound, assess its experimental readiness for lab testing TODAY.

Return a JSON object with these exact keys:
  compound, overall_readiness (0-10 int), solubility, typical_concentration,
  cell_models (array), assays (array), suppliers (array),
  timeline_days (int), blockers (array), next_step

solubility: "Water-soluble" / "Lipid-soluble" / "Both" / "Unknown"
typical_concentration: e.g. "1-10 µM" or "Unknown"
cell_models: list of relevant cell lines used in literature (e.g. HaCaT, fibroblast)
assays: list of relevant assays (e.g. MTT, ELISA, wound scratch)
suppliers: list of 2-3 known commercial suppliers
timeline_days: estimated days to first data point
blockers: list of potential obstacles
next_step: one concrete action sentence

Respond ONLY with valid JSON. No markdown fences."""

CLUSTER_SYSTEM = """You are a cosmetic R&D scientist. Given retrieved literature, identify
and cluster natural compounds by their primary biological mechanism for the requested target.

Return a JSON object where each key is a mechanism name and value is an array of objects:
  { compound, evidence_strength (1-5 int), notes }
            ...
            if no_llm:
                ...
                with st.spinner(f"Ranking compounds for {application}..."):
                    try:
                        ...
                        st.error(f"Error: {e}")
                        st.info("Try reducing K or switching to Claude API for better JSON output.")
            ...
            if no_llm:
                ...
                with st.spinner(f"Generating claims for {compound}..."):
                    try:
                        ...
                        st.info("Try switching to Claude API for more reliable JSON output.")
            ...
            if no_llm:
                ...
                with st.spinner(f"Assessing readiness for {ready_compound}..."):
                    try:
                        ...
                        st.error(f"Error: {e}")
                        st.info("Try Claude API for more reliable JSON output.")
